...
import { WalletErrorType } from '../../src/types/SteroidWalletTypes.js';
// Create mock wallet
function createMockWallet(publicKey) {
...
function createMockSteroidConnection() {
    return {
    ...
    describe('signTransaction', () => {
        ...
        it('should throw NOT_CONNECTED for disconnected wallet', async () => {
            ...
            try {
                ...
                expect(error.type).toBe(WalletErrorType.NOT_CONNECTED);
            }
        });
    });
    ...
    describe('signMessage', () => {
        ...
        it('should throw UNSUPPORTED_OPERATION if wallet does not support signMessage', async () => {
            ...
            try {
                ...
                expect(error.type).toBe(WalletErrorType.UNSUPPORTED_OPERATION);
            }
        });
    });
    describe('Error Normalization', () => {
        it('should normalize user rejection errors', async () => {
            mockWallet.signTransaction = vi.fn().mockRejectedValue(new Error('User rejected the request'));
            const tx = createTestTransaction();
            try {
        ...
        it('should normalize "user denied" errors', async () => {
            mockWallet.signTransaction = vi.fn().mockRejectedValue(new Error('User denied the signature request'));
            const tx = createTestTransaction();
            try {
                await steroidWallet.signTransaction(tx);
            }
        ...
        it('should normalize connection errors', async () => {
            mockWallet.signTransaction = vi.fn().mockRejectedValue(new Error('Wallet not connected'));
            const tx = createTestTransaction();
            try {
                await steroidWallet.signTransaction(tx);
            }
        ...
        it('should normalize signing failure errors', async () => {
            mockWallet.signTransaction = vi.fn().mockRejectedValue(new Error('Failed to sign transaction'));
            const tx = createTestTransaction();
            try {
                await steroidWallet.signTransaction(tx);
            }
            catch (error) {
                expect(error.type).toBe(WalletErrorType.SIGNING_FAILED);
            }
        });
        it('should map unknown errors to UNKNOWN type', async () => {
            mockWallet.signTransaction = vi.fn().mockRejectedValue(new Error('Some completely random error'));
            const tx = createTestTransaction();
            try {
                await steroidWallet.signTransaction(tx);
            }
            catch (error) {
                expect(error.type).toBe(WalletErrorType.UNKNOWN);
...
//# sourceMappingURL=SteroidWallet.test.js.map